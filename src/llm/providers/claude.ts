...
export function createClaudeProvider(input: {
  baseUrl: string;
  model: string;
  apiKeyEnv: string;
}): LLMProvider {
  const baseUrl = normalizeBaseUrl(
    input.baseUrl || "https://api.anthropic.com/v1",
  );
  ...
  if (!model) {
    throw new Error(
      "Claude model is required. Set llm.model or CLAUDE_MODEL/ANTHROPIC_MODEL.",
    );
  }
  ...
  async function chat(
    ...
    const resp = await postJson(`${baseUrl}/messages`, headers, payload);
