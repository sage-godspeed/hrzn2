...
async function resolveCypressSpec(projectRoot: string, testId: string): Promise<string | null> {
...
export function createCypressRunner(input: { projectRoot: string; artifactsDir: string }): E2ERunner {
  return {
    kind: "cypress",
    async detect() {
      const local = resolve(input.projectRoot, "node_modules", ".bin", "cypress");
      return { kind: "cypress", version: (await exists(local)) ? local : undefined };
    },
    ...
    async run(req: RunRequest): Promise<Evidence> {
      ...
      const localBin = resolve(input.projectRoot, "node_modules", ".bin", "cypress");
      ...
      const result = await runProcess(localBin, args, { cwd: input.projectRoot });
      const finishedAt = new Date().toISOString();

      const copied: string[] = [];
      for (const dirName of ["cypress/videos", "cypress/screenshots", "cypress/results"]) {
        copied.push(...(await copyArtifacts(resolve(input.projectRoot, dirName), resolve(runOutDir, dirName))));
      }
      ...
      const artifacts: Evidence["artifacts"] = {
        ...
        traces: [],
        logs: [resolve(runOutDir, "runner.log")]
      };
      ...
      return { startedAt, finishedAt, runner: "cypress", runId, artifacts, failingTests };
    },
