...
import { detectWorkspaceRules, policyFromWorkspaceRules } from "./workspaceRules.ts";
...
function applyTestcaseOverrides(base: SafePolicy, spec: TestcaseSpec): SafePolicy {
  const allowOverrides = (spec.healingPolicy?.allow ?? []).map(asCapability).filter(Boolean) as HealCapability[];
  const denyOverrides = (spec.healingPolicy?.deny ?? []).map(asCapability).filter(Boolean) as HealCapability[];

  const allow = uniq([...base.allow, ...allowOverrides]).filter((c) => !denyOverrides.includes(c));
  const deny = uniq([...base.deny, ...denyOverrides]).filter((c) => !allowOverrides.includes(c));
...
export async function resolvePolicy(input: { projectRoot: string; spec?: TestcaseSpec }) {
  ...
  const effective = input.spec ? applyTestcaseOverrides(workspacePolicy, input.spec) : workspacePolicy;
